...
Hover the mouse pointer over coded text to see infomration of the code and any meno. Coded text can be uncoded by clicking on the text segment and pressing the Unmark button.
...
The _Auto-code exact text_ button and the _Auto-code text fragment_ button have a right-click menu for more options. The default is to auto-code all text. There are options to autocode only the first matching text or the last matching text in the document. Another option is to apply the auto-code only if it appears within another code (for example within a specific coded speakers text). The auto-code exact text and text fragment buttons ask which files you want auto-coded.

There is an undo button to remove the auo-codings that were last applied.
...
- The speaker name must be at the start of a new line and less than 64 characters long. The follwing formats are detected: "Name:", "{Name} or "[Name]"
...
- Each speaker will get it's own code in the special catagory "📌 Speakers" at the very bottom of the code tree. The default colors are from the gray spectrum, to make them less distracting.
...
It is always best to save a copy of the project before editing the text, to avoid the need to edit coding positions, in case after editing text the coding positions are shifted incorrectly. The below key strokes may help to recover the correct positions.
