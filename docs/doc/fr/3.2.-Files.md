...
Importez du texte depuis des documents en texte brut, docx, epub, md, odt et pdf. Les formats d'image suivants peuvent être chargés : jpg, jpeg et png. Les formats audio/vidéo suivants peuvent être utilisés : mp3, m4a, mp4, wav, mov, ogg. Les PDF peuvent être codés en tant que texte ou image.
...
# Données Twitter (maintenant X)

**Note :** La fonction Twitter n'a pas été révisée récemment et peut ne pas fonctionner.

Importez des données Twitter depuis un fichier CSV entièrement cité. Un fichier d'exemple **« rtweet_judo_tweets_data.csv »** se trouve dans le dossier **« Examples »**.

Le fichier CSV nécessite ces noms de colonnes exacts :
- **« id »** et **« full_text »** pour les données de tweet
- **« screen_name »** pour les données utilisateur

Des champs de tweet supplémentaires peuvent utiliser ces noms de colonnes exacts :
- **« created_at »**, **« coordinates »**, **« retweet_count »**, **« favorite_count »**, **« lang »**
Des champs utilisateur supplémentaires peuvent être :
- **« location »**, **« url »**, **« description »**, **« followers_count »**, **« friends_count »**, **« listed_count »**, **« favourites_count »**, **« statuses_count »**

La page d'importation Twitter contient également un exemple d'utilisation de **R** avec **RTweet** pour extraire des données de tweet. Cela a été testé et fonctionnait avant que Twitter n'applique des frais pour ce service. Notez qu'il y a actuellement un coût associé à l'accès aux données Twitter. Je ne peux pas donner de conseils sur les problèmes que vous pourriez rencontrer pour obtenir des données Twitter.
