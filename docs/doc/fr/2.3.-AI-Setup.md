...
Après l'installation de QualCoder, les fonctionnalités améliorées par l'IA nécessitent une configuration supplémentaire. Lors du premier lancement de l'application, un **assistant** apparaît et vous guide tout au long du processus. Si vous choisissez de sauter cette étape, vous pourrez relancer l'assistant plus tard en sélectionnant l'option de menu **« IA > Assistant de configuration »**.
...
* **Pour d'autres options de fournisseurs d'IA, [voir ci-dessous](#utiliser-dautres-modèles-dia).**
...
Une fois toutes ces étapes terminées, la barre d'état de l'application affichera **« IA : prêt »**, et vous pourrez commencer à utiliser le [chat IA](https://github.com/ccbogel/QualCoder/wiki/5.1.-AI-chat-based-analysis) ou le [codage assisté par IA](https://github.com/ccbogel/QualCoder/wiki/4.2.-AI-Assisted-Coding) pour explorer vos données.
...
Google offre différents modèles payants et gratuits sur ses serveurs. Example config for Gemini 1.5 flash:
```
