...
* Comparaison de codage - pour les fichiers texte (compare 2 codeurs)
* Comparaison de codage par fichier - pour les fichiers texte (compare 2 codeurs)
...
Cliquez avec le bouton droit sur un en-tête de code dans le rapport généré pour afficher d'autres options de menu : Afficher dans le contexte, désélectionner, modifier le code.
...
Cette option affiche les similitudes et les différences globales entre deux codeurs dans tous les fichiers texte. Sélectionnez deux codeurs et cliquez sur le bouton « Exécuter les comparaisons ». Les statistiques créées ici concernent uniquement le texte codé, et non les images ou les fichiers multimédias codés. 
...
Le pourcentage de désaccord indique le pourcentage de tout le texte codé et non codé qui ne correspondait pas entre les codeurs. Il est égal à 100 moins le pourcentage de concordance.
...
Le coefficient Kappa de Cohen est calculé à partir des informations disponibles sur Wikipédia https://en.wikipedia.org/wiki/Cohen%27s_kappa
...
Boutons permettant d'effectuer les comparaisons, d'effacer les codeurs sélectionnés, d'exporter vers Excel et d'ouvrir une fenêtre d'aide sur les statistiques.

#  Coding Comparison by file Report

Cette option affiche les similitudes et les différences globales entre deux codeurs dans UN fichier sélectionné. Sélectionnez un fichier, un code et deux codeurs. Cliquez ensuite sur le bouton « Exécuter les comparaisons ». Les statistiques et les comparaisons pour les segments audio/vidéo codés sont en cours de développement. 
La concordance globale, la discordance et le coefficient Kappa de Cohen sont calculés pour le texte et les images. Un fichier image s'affiche dans une fenêtre séparée, montrant les sélections des deux codeurs dans des zones encadrées en jaune ou en bleu. Le texte en vert indique les endroits où les deux codeurs ont codé la même section de texte. Le survol de la souris fournit des informations supplémentaires.
...
Cette fenêtre affiche tous les codes et catégories ainsi que la fréquence d'utilisation pour chaque codeur. Les fréquences des codes peuvent être sélectionnées pour des fichiers spécifiques.
...
Dans la version 3.8 et les versions ultérieures, vous pouvez cliquer avec le bouton droit de la souris sur une cellule contenant du texte codé par cooccurrence pour créer un nouveau code fusionné qui combine les deux codes qui se chevauchent.
...
#  Codes par segments de texte Rapport
...
Il est possible de créer des graphiques circulaires, à barres, en rayons et arborescents.

À l'exception du WordCloud, les graphiques s'affichent dans le navigateur Web par défaut. Si votre navigateur n'affiche pas votre graphique, commencez par changer votre navigateur Web par défaut pour un autre navigateur récent, par exemple Chrome, Edge ou Safari. Si vous avez désactivé javascript, les graphiques ne s'afficheront pas.
...
![Graphique à barres](https://qualcoder.files.wordpress.com/2022/04/pic11.png)

_Graphique en rayons de soleil_

![Graphique en rayons de soleil](https://qualcoder.files.wordpress.com/2022/04/pic10.png)
...
![Nuage de mots](https://qualcoder.wordpress.com/wp-content/uploads/2023/12/wc.png)
...
Cliquez avec le bouton droit de la souris dans le tableau des résultats pour accéder aux options de filtrage.
...
Le sql stocké correspond aux instructions sql que vous avez créées et stockées. Pour stocker une instruction sql, cliquez avec le bouton droit sur le sql et choisissez « enregistrer la requête ».
