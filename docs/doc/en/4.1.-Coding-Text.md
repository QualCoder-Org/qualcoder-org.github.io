...
Create a new code by right-clicking in the left hand window. Codes can be assigned a colour by right-clicking on the code and selecting the change code colour option. There is a button (to right with artist palette image) that will set all newly created coded to your select colour, instead of random colour assignment.

Other options from the right-click menu include adding a memo to the code, deleting the code, renaming, adding a new code, and adding a new category, showing codes like (a certain name). 
...
Coded text segments can be memoed - through the right-click menu, by pressing 'm' or by clicking on the 'notepad and pencil' icon. Coded and memoed text is italicised.
...
Overlapping codes can be difficult to view clearly. Overlapping sections are underlined. Mouse hover will show coded text, including overlaps. You can press 'O' after clicking on an overlapping section to cycle through the codes at that location.
...
The _Auto-code exact text_ button has a right-click menu for more options. The default is to auto-code all text. There are options to autocode only the first matching text or the last matching text in the document. The auto-code exact text asks which files you want auto-coded, so this is a useful option of you want to identify only one instance across many files.
