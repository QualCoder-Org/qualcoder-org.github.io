...
Each file is preceded by an icon that describes the type of file. Other columns show the file name, date of creation/upload, association with cases, and other attributes. 
...
Text files containing emoticons can be imported and displayed. However, emoticons can 'offset' subsequent coding strips, by 1 character per emoticon. 
...
Older projects extracted text differently from version 4.0. So When opening an older project, the 4.0 version will try and match the text segements as best as possible. however, it may not be identical.

### Pseudonymisation (data de-identification)

Pseudonymisation is available to replace words or phrases. For many languages (possibly except Chinese, Japanese, Korean), providing the character before the phrase and the character after the phrase are non-alphabetical and non-numerical, this is an easy cut-off to enact. Pseudonymisation is case-sensitive. Original words must be at least 2 characters long, and pseudonyms must be at least 3 characters long. Alternatively, leave the pseudonym entry blank for a randomly generated term.

An 'Anonymise on import' button is accessed in Manage files. This allows you to set up a list of original terms to pseudonyms, and must be in place before any importing text documents. It does not apply retrospectively to already imported documents.
...
PDF files are not pseudonymised.
...
Filename entries can be changed in the database. The file names of the original files will not be changed. A right-click context menu allows you to view, export, delete a current file. The trashcan button is an option to select multiple files for deletion.  The question mark '?' opens this help page. Files can be viewed either by right-click menu on the file, or by clicking the magnifying glass icon. This is where transcripts for audio and video can be entered, when viewing an audio or video file.
...
Attributes are variables to describe the files. These can be added using the (x) button or through the Manage Attributes menu option. They can be character or numeric. The above image shows an added character attribute called _origin_ as well as one of several reference attribute columns _Ref-author_.
...
There are sample files in the Examples folder. These can be used to test importing files of different document formats and of importing an image. There are also example files to test importing case attributes, and for importing a survey.
