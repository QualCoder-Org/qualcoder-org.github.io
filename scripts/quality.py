...
def fix_headings(text: str) -> tuple[str, int]:
    """Fix heading structure: ensure only one H1 (#), leave everything else unchanged.
    
    Rules:
    - Keep only the first H1 (#), remove the # from subsequent H1s (keep text as paragraph)
    - Do NOT modify other heading levels (H2-H6)
    ...
    for line in lines:
        ...
        if m:
            ...
            if level == 1:
                if h1_found:
                    # Remove the # to make it a regular paragraph
                    new_line = title
                    new_lines.append(new_line)
                    count += 1
                    continue
